...
describe("subagent-resolver", () => {
  ...
  describe("resolveSubagents", () => {
    ...
    it("should read agentType from meta.json", () => {
      const agentFile = join(tempDir, "agent-meta1.jsonl")
      const metaFile = join(tempDir, "agent-meta1.meta.json")
      writeFileSync(
        agentFile,
        '{"type":"user","timestamp":"2026-05-07T19:22:45.000Z","message":{"role":"user","content":"test"}}\n{"type":"assistant","timestamp":"2026-05-07T19:22:46.000Z","message":{"role":"assistant","content":[{"type":"text","text":"Done"}]}}\n',
      )
      writeFileSync(metaFile, JSON.stringify({ agentType: "Explore", description: "Explore the codebase" }))

      const subagentFiles = [
        { filePath: agentFile, agentId: "meta1", isNewStructure: true },
      ]

      const result = resolveSubagents(subagentFiles, [])
      expect(result).toHaveLength(1)
      expect(result[0].agentType).toBe("Explore")
      // meta.json description should override JSONL-extracted description
      expect(result[0].description).toBe("Explore the codebase")
    })

    it("should fall back to JSONL description when meta.json is missing", () => {
      const agentFile = join(tempDir, "agent-nometa.jsonl")
      writeFileSync(
        agentFile,
        `${JSON.stringify({ type: "user", timestamp: "2026-05-07T19:22:45.000Z", message: { role: "user", content: "test" } })}\n${JSON.stringify({ type: "assistant", timestamp: "2026-05-07T19:22:46.000Z", message: { role: "assistant", content: [{ type: "text", text: "JSONL description" }] } })}\n`,
      )

      const subagentFiles = [
        { filePath: agentFile, agentId: "nometa", isNewStructure: true },
      ]

      const result = resolveSubagents(subagentFiles, [])
      expect(result).toHaveLength(1)
      expect(result[0].agentType).toBeUndefined()
      expect(result[0].description).toBe("JSONL description")
    })

    it("should calculate totalCost from tokens and model", () => {
      const agentFile = join(tempDir, "agent-cost.jsonl")
      writeFileSync(
        agentFile,
        `${JSON.stringify({
          type: "assistant",
      ...
      const subagentFiles = [
        { filePath: agentFile, agentId: "cost", isNewStructure: true },
      ]

      const result = resolveSubagents(subagentFiles, [])
      expect(result).toHaveLength(1)
      expect(result[0].totalCost).toBeGreaterThan(0)
      expect(result[0].model).toContain("haiku")
    })
  })
})

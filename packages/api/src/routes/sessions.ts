...
export function createSessionsRouter(config: Config, cache: SessionCache): Router {
  ...
  router.get("/sessions", async (req, res) => {
    ...
    try {
      ...
      // Enrich with API cost data from cost-stream.db
      let costStreamDb: { getCostSummariesForSessions(ids: string[]): Map<string, number>; close(): void } | null = null
      if (existsSync(config.costStreamDbPath)) {
        try {
          const { CostStreamDb } = await import(
            "claude-timeline/cost-stream-db" as string
          )
      ...
      const sessionIds = merged.map((s) => s.sessionId)
      const apiCosts = costStreamDb?.getCostSummariesForSessions(sessionIds) ?? new Map()

      const enriched = merged.map((s) => ({
        ...s,
        apiTotalCost: apiCosts.get(s.sessionId) ?? null,
      }))
    ...
    try {
      ...
      try {
        const { enrichTimelineWithCostStream } = await import(
          "claude-timeline/cost-stream-merger" as string
        )
  ...
  router.post("/sessions/refresh", async (_req, res) => {
    ...
    try {
      ...
      const refreshSessionIds = merged.map((s) => s.sessionId)
      const apiCosts = costStreamDb?.getCostSummariesForSessions(refreshSessionIds) ?? new Map()
